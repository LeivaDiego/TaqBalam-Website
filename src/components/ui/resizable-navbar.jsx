	...
	return (
		<motion.div
			...
			className={cn(
				...
				self-start rounded-full bg-transparent px-4 py-2 lg:flex dark:bg-transparent`,
...
export const NavItems = ({ items, className, onItemClick }) => {
	...
	return (
		<motion.div
			...
			className={cn(
				`absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm
				font-medium text-zinc-600 transition hover:text-zinc-800 lg:flex`,
				className,
			)}
		>
			{items.map((item, idx) => {
...
export const MobileNav = ({ children, className, visible }) => {
	return (
		<motion.div
			...
			className={cn(
				`relative z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] flex-col items-center
				justify-between bg-transparent px-0 py-2 lg:hidden`,
				visible && 'bg-white/80 dark:bg-neutral-950/80',
				className,
			)}
		>
		...
		<AnimatePresence>
			{isOpen && (
				<motion.div
					...
					exit={{ opacity: 0 }}
					className={cn(
						`absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4
						rounded-lg bg-white px-4 py-8
